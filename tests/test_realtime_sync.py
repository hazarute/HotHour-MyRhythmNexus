...
from app.services.socket_service import emit_price_update, emit_turbo_triggered, emit_auction_booked
...
async def test_realtime_price_and_turbo_sync_to_multiple_clients(live_server_url: str):
    ...
    client_one_events = {"price_update": [], "turbo_triggered": [], "auction_booked": []}
    client_two_events = {"price_update": [], "turbo_triggered": [], "auction_booked": []}
    ...
    @client_one.on("auction_booked")
    async def _client_one_booked(payload):
        client_one_events["auction_booked"].append(payload)

    @client_two.on("auction_booked")
    async def _client_two_booked(payload):
        client_two_events["auction_booked"].append(payload)
    ...
    await emit_auction_booked(
        auction_id=auction_id,
        booking_code="HOT-RTSYNC",
    )
    ...
    assert len(client_one_events["auction_booked"]) == 1
    assert len(client_two_events["auction_booked"]) == 1
    ...
    assert client_one_events["auction_booked"][0]["auction_id"] == auction_id
    assert client_two_events["auction_booked"][0]["auction_id"] == auction_id
